...
@cached
def _adjust_angle(is_mirrored: bool, max_angle: int, angle: int, deviation: int, min_angle: int) -> int:
    """
...
class SServo:
    ...
    def set(self, target_angle: int, duration: float, nm_action: bool = False) -> None:
        ...
        adjusted_target = _adjust_angle(is_mirrored=self.mirrored, max_angle=self.max_angle, min_angle=self.min_angle, angle=target_angle, deviation=self.deviation)
    ...
    def set_straight(self, angle: int) -> None:
        """Set the servo to a straight angle."""
        with self.lock:
            adjusted_target = _adjust_angle(is_mirrored=self.mirrored, max_angle=self.max_angle, min_angle=self.min_angle, angle=angle, deviation=self.deviation)

            if not self.min_angle <= adjusted_target <= self.max_angle:
                raise ValueError(f"Adjusted target angle {adjusted_target} is out of range [{self.min_angle} - {self.max_angle}]")
            elif self.servo.angle == adjusted_target:
                return

            def move() -> None:
                self.servo.angle = adjusted_target

            self.servo_thread = Thread(target=move)
